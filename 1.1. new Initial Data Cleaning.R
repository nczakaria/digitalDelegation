...
filter_country_data <- function(data, country) {
  return(subset(data, !(Country == country)))
}
...
my.dat <- transform_factor(my.dat, "FinanceLoginWays_institution", na = "2", levels = c("No", "Yes"), labels = c("0", "1"), convert_to_numeric = TRUE)
...
my.dat <- transform_factor(my.dat, "DigitalAssistAccessWays_institution", na = "2", levels = c("No", "Yes"), labels = c("0", "1"), convert_to_numeric = TRUE)
...
process_data <- function(data, country) {
  ...
  highstake_columns <- c("MedicalDecisionPerson", "MedicalDecisionDoctor", "MedicalSignPapers", "MedicalWorthPerson", "MedicalWorthDoctor", "MedicalWorthFamily", "MedicalEndPerson", "MedicalEndFamily", "MedicalFlexibilityPerson", "MedicalFlexibilityDoctor", "MedicalFlexibilityFamily", "MedicalDoctorQuestions")
